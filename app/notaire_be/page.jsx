...
export default function Notaire_be() {
    ...
    const handleScrape = async () => {
        ...
        try {
            ...
            const { hrefs, nom_etude } = result.data;

            
            

            hrefs.forEach(async (href, index) => {
                const response = await fetch(`/api/notaire_be/get_address?url=${encodeURIComponent(href)}`);
                if (!response.ok) {
                    throw new Error(`Error: ${response.statusText}`);
                }
                const result = await response.json();
                const { newAddresses } = result.data;
                setData(prev => [...prev, { nom_etude: nom_etude[index], adresse_etude: newAddresses[0] }]);
                            ...
                            <tbody>
                                { data.map((item, index) => (
                                    <tr key={index} className={index % 2 === 0 ? "bg-gray-50" : ""}>
                                        ...
                                        <td className="border px-4 py-2">{item.nom_etude}</td>
                                        <td className="border px-4 py-2">{item.adresse_etude}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
